...
(async()=>{
  ...
  try{
    ...
    const root = await sftp.list('/home/u970251027/domains/shelbyimportacionessas.com/public_html');
    console.log('public_html ->', root.map(x=>x.name));
    const assets = await sftp.list('/home/u970251027/domains/shelbyimportacionessas.com/public_html/assets');
